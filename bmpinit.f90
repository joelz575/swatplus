...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      if (dtp_onoff(i)==1) then
         ...
         if (dtp_iyr(i)<=1000)   dtp_iyr(i) = time%yrc
   ...
   !!            stop
      ...
      if (wtp_onoff(i)==1) then
         ...
         if (wtp_iyr(i)<=1000)   wtp_iyr(i) = time%yrc
      ...
      do k=1,num_ri(i)
	   ...
	   ri_ndt(i,k) = (ri_dd(i,k) - 12.) * 60 / time%dtm  !minus the first dry 12 hours no-pumping 
	   ...
	   ri_pumpv(i,k) = ri_vol(i,k) / ri_ndt(i,k) !m3/dt
         ...
         if (ri_iy(i,k)==0) ri_iy(i,k) = time%yrc
...
!               wqv/(4.+1.33*4.) * 0.093
         ...
         if (sf_iy(i,k)==0) sf_iy(i,k) = time%yrc

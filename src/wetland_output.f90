...
!!!!! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%res%d == "y") then
            write (2548,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, wet_wat_d(j),&
                    wet(j), wet_in_d(j), wet_out_d(j)
...
!!!!! monthly print
        if (time%end_mo == 1) then
          ...
          if (pco%res%m == "y") then
            write (2549,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, wet_wat_m(j),&
                    wet(j), wet_in_m(j), wet_out_m(j)
              if (pco%csvout == "y") then
                write (2553,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                        ob(iob)%name, wet_wat_m(j), wet(j), wet_in_m(j), wet_out_m(j)
              end if 
          end if
          wet_in_m(j) = resmz
          wet_out_m(j) = resmz
...
!!!!! yearly print
       if (time%end_yr == 1) then
          ...
          if (pco%res%y == "y") then
            write (2550,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, wet_wat_y(j),&
                    wet(j), wet_in_y(j), wet_out_y(j)
              if (pco%csvout == "y") then
                write (2554,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                        ob(iob)%name, wet_wat_y(j), wet(j), wet_in_y(j), wet_out_y(j)
              end if
          end if
          wet_in_y(j) = resmz
          wet_out_y(j) = resmz
...
!!!!! average annual print
        if (time%end_sim == 1 .and. pco%res%a == "y") then
          ...
          write (2551,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, wet_wat_a(j),&
                  wet(j), wet_in_a(j), wet_out_a(j)
          if (pco%csvout == "y") then
            write (2555,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&

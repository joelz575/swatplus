      ...
      do ilsu = 1, db_mx%lsu_out
        ...
        !! daily print - LANDSCAPE UNIT
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%wb_lsu%d == "y") then
            write (2140,100) time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_d(ilsu)  !! waterbal
              if (pco%csvout == "y") then
                write (2144,'(*(G0.3,:","))') &
                        time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_d(ilsu)  !! waterbal
              end if
          end if
          if (pco%nb_lsu%d == "y") then
            write (2150,103) &
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_d(ilsu)  !! nutrient bal
            if (pco%csvout == "y") then
              write (2154,'(*(G0.3,:","))') &
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_d(ilsu)  !! nutrient bal
            end if
          ...
          if (pco%ls_lsu%d == "y") then
            write (2160,100) &
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_d(ilsu)  !! losses
            if (pco%csvout == "y") then
              write (2164,'(*(G0.3,:","))') &
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_d(ilsu)  !! losses
            end if
          ...
          if (pco%pw_lsu%d == "y") then
            write (2170,100)&
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_d(ilsu)  !! plant weather
            if (pco%csvout == "y") then
              write (2175,'(*(G0.3,:","))')&
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_d(ilsu)  !! plant weather
            end if
        ...
        if (time%end_mo == 1) then
          ...
          if (pco%wb_lsu%m == "y") then
            write (2141,100)&
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_m(ilsu)
            if (pco%csvout == "y") then
              write (2145,'(*(G0.3,:","))')&
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_m(ilsu)
            end if
          ...
          if (pco%nb_lsu%m == "y") then
            write (2151,103)&
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_m(ilsu)
            if (pco%csvout == "y") then
              write (2155,'(*(G0.3,:","))')&
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_m(ilsu)
            end if
          ...
          if (pco%ls_lsu%m == "y") then
            write (2161,100)&
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_m(ilsu)
            if (pco%csvout == "y") then
              write (2165,'(*(G0.3,:","))')&
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_m(ilsu)
            end if
          ...
          if (pco%pw_lsu%m == "y") then
            write (2171,100)&
                    time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_m(ilsu)
            if (pco%csvout == "y") then
              write (2175,'(*(G0.3,:","))')&
                      time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_m(ilsu)
            end if
        ...
        if (time%end_yr == 1) then
           ...
           if (pco%wb_lsu%y == "y") then
             write (2142,102)&
                     time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_y(ilsu)
             if (pco%csvout == "y") then
               write (2146,'(*(G0.3,:","))')&
                       time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_y(ilsu)
             end if
           ...
           if (pco%nb_lsu%y == "y") then
             write (2152,103)&
                     time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_y(ilsu)
             if (pco%csvout == "y") then
               write (2156,'(*(G0.3,:","))')&
                       time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_y(ilsu)
             end if
           ...
           if (pco%ls_lsu%y == "y") then
             write (2162,102)&
                     time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_y(ilsu)
             if (pco%csvout == "y") then
               write (2166,'(*(G0.3,:","))')&
                       time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_y(ilsu)
             end if
           ...
           if (pco%pw_lsu%y == "y") then
             write (2172,102)&
                     time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_y(ilsu)
             if (pco%csvout == "y") then
               write (2176,'(*(G0.3,:","))')&
                       time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_y(ilsu)
             end if
      ...
      if (time%end_sim == 1 .and. pco%wb_lsu%a == "y") then
        ...
        if (pco%csvout == "y") then
          write (2147,'(*(G0.3,:","))')&
                  time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruwb_a(ilsu)
        end if
      end if
      if (time%end_sim == 1 .and. pco%nb_lsu%a == "y") then
        runb_a(ilsu) = runb_a(ilsu) / time%yrs_prt
        write (2153,103)&
                time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_a(ilsu)
        if (pco%csvout == "y") then
          write (2157,'(*(G0.3,:","))')&
                  time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, runb_a(ilsu)
        end if
      ...
      if (time%end_sim == 1 .and. pco%ls_lsu%a == "y") then
        ...
        if (pco%csvout == "y") then
          write (2167,'(*(G0.3,:","))')&
                  time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, ruls_a(ilsu)
        end if
      end if
      if (time%end_sim == 1 .and. pco%pw_lsu%a == "y") then
        ...
        if (pco%csvout == "y") then
          write (2177,'(*(G0.3,:","))')&
                  time%day, time%mo, time%day_mo, time%yrc, ilsu, "       0", lsu_out(ilsu)%name, rupw_a(ilsu)
        end if
      end if
      end do    !ilsu

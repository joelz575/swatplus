      ...
      if (i_exist /= 0 .or. in_delr%path /= 'null') then
        do
          ...
          do ii = 1, db_mx%dr_path
            read (107,*,iostat=eof) titldum
            backspace (107)
            read (107,*,iostat=eof) dr_path_name(ii), (dr_path(ii)%path(ipath)%sol, dr_path(ii)%path(ipath)%sor, ipath = 1, cs_db%num_paths)   
            if (eof < 0) exit
          end do
          close (107)
          exit

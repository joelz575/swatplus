      ...
      do icmd = 1, sp_ob%objs
        ...
        !! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%hyd%d == "y") then
             write (2560,*) time%day, time%mo, time%day_mo, time%yrc, ob(icmd)%name, ob(icmd)%typ,&
                     ob(icmd)%obtyp_in(iin),        &
...
!              ob(icmd)%obtypno_in(iin), ob(icmd)%htyp_in(iin), ob(icmd)%frac_in(iin), ob(icmd)%hin_d(iin)
        ...
        if (time%end_mo == 1) then
          if (pco%hyd%m == "y") then
            write (2561,*) time%day, time%mo, time%day_mo, time%yrc, ob(icmd)%name, ob(icmd)%typ,&
                    ob(icmd)%obtyp_in(iin),        &
        ...
        if (time%end_yr == 1) then
          if (pco%hyd%y == "y") then
            write (2562,*) time%day, time%mo, time%day_mo, time%yrc, ob(icmd)%name,  ob(icmd)%typ,&
                    ob(icmd)%obtyp_in(iin),        &
             ob(icmd)%obtypno_in(iin), ob(icmd)%htyp_in(iin), ob(icmd)%frac_in(iin), ob(icmd)%hin_y(iin)
            if (pco%csvout == "y") then
              write (2566,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, ob(icmd)%name,  ob(icmd)%typ,&
                      ob(icmd)%num, ob(icmd)%obtyp_in(iin),        &
               ob(icmd)%obtypno_in(iin), ob(icmd)%htyp_in(iin), ob(icmd)%frac_in(iin), ob(icmd)%hin_y(iin)
            endif
          end if
        ...
        if (time%end_sim == 1 .and. pco%hyd%a == "y") then
          ...
          write (2563,*) time%day, time%mo, time%day_mo, time%yrc, ob(icmd)%name,  ob(icmd)%typ,&
                  ob(icmd)%obtyp_in(iin),        &

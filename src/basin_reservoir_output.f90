      ...
      real :: const
        ...
        if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%res_bsn%d == "y") then
            write (2100,100) time%day, time%mo, time%day_mo, time%yrc, ires, "     1", bsn%name, bres_wat_d, bres,&
                    bres_in_d, bres_out_d
        ...
        if (time%end_yr == 1) then
          ...
          if (pco%res_bsn%y == "y") then
            write (2102,100) time%day, time%mo, time%day_mo, time%yrc, ires, "     1", bsn%name,  bres_wat_y, bres,&
                    bres_in_y, bres_out_y
            if (pco%csvout == "y") then
      ...
      if (time%end_sim == 1 .and. pco%res_bsn%a == "y") then
        ...
        if (pco%csvout == "y") then 
          write (2107,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, ires, "     1", bsn%name, bres_wat_a,&
                  bres, bres_in_a, bres_out_a
        end if 
      end if

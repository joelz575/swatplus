...
!      end if
      ...
      if (sp_ob%hru > 0) then
        call connect_read(in_con%hru_con, "hru     ", sp_ob1%hru, sp_ob%hru, 5, 2)     
      end if
      
      !read connect file for hru_lte's
      if (sp_ob%hru_lte > 0) then
        call connect_read(in_con%hruez_con, "hru_lte ", sp_ob1%hru_lte, sp_ob%hru_lte, 1, 2) 
      end if
      ...
      if (sp_ob%modflow > 0) then
        call connect_read(in_con%modflow_con, "modflow ", sp_ob1%modflow, sp_ob%modflow, 1, 0)
      end if
                     
      !read connect file for aquifer (1-D)
      if (sp_ob%aqu > 0) then
        call connect_read(in_con%aqu_con, "aqu     ", sp_ob1%aqu, sp_ob%aqu, 5, 0)
        call aqu2d_read
      end if
                  
      !read connect file for channels
      if (sp_ob%chan > 0) then
        call connect_read(in_con%chan_con, "chan    ", sp_ob1%chan, sp_ob%chan, 3, 3) 
        call overbank_read
      end if
                                  
      !read connect file for reservoirs
      if (sp_ob%res > 0) then
        call connect_read(in_con%res_con, "res     ", sp_ob1%res, sp_ob%res, 2, 1) 
      end if
                
      !read connect file for recalls
      if (sp_ob%recall > 0) then
        call connect_read(in_con%rec_con, "recall  ", sp_ob1%recall, sp_ob%recall, 1, 0) 
        call recall_read
      end if
                
      !read connect file for export coefficients
      if (sp_ob%exco > 0) then
        call connect_read(in_con%exco_con, "exco    ", sp_ob1%exco, sp_ob%exco, 2, 0) 
        call exco_read    
      endif
                  
      !read connect file for delivery ratio
      if (sp_ob%dr > 0) then
        call connect_read(in_con%delr_con, "dr      ", sp_ob1%dr, sp_ob%dr, 2, 0) 
        call dr_read
      end if

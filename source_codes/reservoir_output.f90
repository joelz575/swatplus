...
!!!!! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%res%d == "y") then
            write (2540,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_d(j),&
                    res(j), res_in_d(j), res_out_d(j)
...
!!!!! monthly print
        if (time%end_mo == 1) then
          ...
          if (pco%res%m == "y") then
            write (2541,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_m(j),&
                    res(j), res_in_m(j), res_out_m(j)
              if (pco%csvout == "y") then
                write (2545,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                        ob(iob)%name, res_wat_m(j), res(j), res_in_m(j), res_out_m(j)
              end if 
          end if
          res_in_m(j) = resmz
          res_out_m(j) = resmz
...
!!!!! yearly print
       if (time%end_yr == 1) then
          ...
          if (pco%res%y == "y") then
            write (2542,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_y(j),&
                    res(j), res_in_y(j), res_out_y(j)
              if (pco%csvout == "y") then
                write (2546,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                        ob(iob)%name, res_wat_y(j), res(j), res_in_y(j), res_out_y(j)
              end if
          end if
          res_in_y(j) = resmz
          res_out_y(j) = resmz
...
!!!!! average annual print
        if (time%end_sim == 1 .and. pco%res%a == "y") then
          ...
          write (2543,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_a(j),&
                  res(j), res_in_a(j), res_out_a(j)
          if (pco%csvout == "y") then

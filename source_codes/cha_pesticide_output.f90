      ...
      !! daily print
        if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%pest%d == "y") then
             write (2808,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                     cs_db%pests(ipest), chpst_d(j)%pest(ipest)   !! pesticide balance
        ...
        if (time%end_mo == 1) then
          ...
          !! monthly print
           if (pco%pest%m == "y") then
             write (2809,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), chpst_m(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2813,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), chpst_m(j)%pest(ipest)
               end if
           end if
        ...
        if (time%end_yr == 1) then
          ...
          !! yearly print
           if (time%end_yr == 1 .and. pco%pest%y == "y") then
             write (2810,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                     cs_db%pests(ipest), chpst_y(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2814,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), chpst_y(j)%pest(ipest)
               end if
           end if
...
!!!!! average annual print
         if (time%end_sim == 1 .and. pco%pest%a == "y") then
           ...
           chpst_a(j)%pest(ipest) = chpst_a(j)%pest(ipest) // time%yrs_prt
           write (2811,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&

...
!! BASIN SWAT DEG CHANNEL OUTPUT
        if (pco%sd_chan_bsn%d == "y") then
          open (2120,file="channel_day_sd_bsn.txt", recl = 1500)
          write (2120,*) bsn%name, prog
          write (2120,*) sdch_hdr
          write (9000,*) "BASIN SWAT DEG CHAN channel_day_sd_bsn.txt"
          if (pco%csvout == "y") then 
            open (2124,file="channel_day_sd_bsn.csv", recl = 1500)
            write (2124,*) bsn%name, prog
            write (2124,'(*(G0.3,:","))') sdch_hdr
            write (9000,*) "BASIN SWAT DEG CHAN channel_day_sd_bsn.csv"
          end if
        endif
        
       if (pco%sd_chan_bsn%m == "y") then
        open (2121,file="channel_mon_sd_bsn.txt",recl = 1500)
        write (2121,*) bsn%name, prog
        write (2121,*) sdch_hdr
        write (9000,*) "BASIN SWAT DEG CHAN channel_mon_sd_bsn.txt"
         if (pco%csvout == "y") then 
           open (2125,file="channel_mon_sd_bsn.csv",recl = 1500)
           write (2125,*) bsn%name, prog
           write (2125,'(*(G0.3,:","))') sdch_hdr 
           write (9000,*) "BASIN SWAT DEG CHANNEL               channel_mon_sd_bsn.txt"
         end if
        end if
       
        if (pco%sd_chan_bsn%y == "y") then
          open (2122,file="channel_yr_sd_bsn.txt", recl = 1500)
          write (2122,*) bsn%name, prog
          write (2122,*) sdch_hdr 
          write (9000,*) "BASIN SWAT DEG CHAN channel_yr_sd_bsn.txt"
          if (pco%csvout == "y") then 
            open (2126,file="channel_yr_sd_bsn.csv", recl = 1500)
            write (2126,*) bsn%name, prog
            write (2126,'(*(G0.3,:","))') sdch_hdr
            write (9000,*) "BASIN SWAT DEG CHAN channel_yr_sd_bsn.csv"
          end if
        endif
        
        if (pco%sd_chan_bsn%a == "y") then
          open (2123,file="channel_aa_sd_bsn.txt",recl = 1500)
          write (2123,*) bsn%name, prog
          write (2123,*) sdch_hdr 
          write (9000,*) "BASIN SWAT DEG CHAN channel_aa_sd_bsn.txt"
          if (pco%csvout == "y") then 
            open (2127,file="channel_aa_sd_bsn.csv",recl = 1500)
            write (2127,*) bsn%name, prog
            write (2127,'(*(G0.3,:","))') sdch_hdr 
            write (9000,*) "BASIN SWAT DEG CHAN channel_aa_sd_bsn.csv"
          end if
        end if
!! BASIN SWAT DEG CHANNEL OUTPUT

      ...
      !! daily print
        if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%pest%d == "y") then
             write (2816,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), respst_d(j)%pest(ipest)   !! pesticide balance
        ...
        if (time%end_mo == 1) then
          ...
          !! monthly print
           if (pco%pest%m == "y") then
             write (2817,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), respst_m(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2821,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), respst_m(j)%pest(ipest)
               end if
           end if
        ...
        if (time%end_yr == 1) then
          ...
          !! yearly print
           if (time%end_yr == 1 .and. pco%pest%y == "y") then
             write (2818,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), respst_y(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2822,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), respst_y(j)%pest(ipest)
               end if
           end if
...
!!!!! average annual print
         if (time%end_sim == 1 .and. pco%pest%a == "y") then
           ...
           write (2819,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&

...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: totbactp         !10^4 cfu      |mass persistent bacteria
      ...
      integer :: ibac          !units         |description
...
!     skipping hourly bacteria route for now  04/16/07 nubs
      if (time%step > 0) then                !! hourly mass balance
        ...
        do ii = 1, time%step
          ...
          totbactp = ob(icmd)%ts(1,ii)%bacp * ob(icmd)%ts(1,ii)%flo + initp * hrchwtr(ii)

          !! compute bacteria die-off
          totbactp = totbactp * Exp(-Theta(bac_db(ibac)%do_stream/24.,bac_db(ibac)%t_adj,wtmp))
      ...
      !! daily mass balance
      !! total bacteria mass in reach
      totbactp = ob(icmd)%hin%bacp * ob(icmd)%hin%flo + ch(jrch)%bactp * rchwtr
      totbactlp = ob(icmd)%hin%baclp * ob(icmd)%hin%flo + ch(jrch)%bactlp * rchwtr

      !! compute bacteria die-off
      !! calculate flow duration
      tday = rttime / 24.0
      if (tday > 1.0) tday = 1.0
      totbactp = totbactp * Exp(-Theta(bac_db(ibac)%do_stream,bac_db(ibac)%t_adj,wtmp)*tday)
      totbactp = Max(0., totbactp)

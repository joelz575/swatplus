      ...
      iob = sp_ob1%aqu + iaq - 1
         ...
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%aqu%d == "y") then
            ...
            if (pco%csvout == "y") then
              write (2524,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, iaq, ob(iob)%gis_id,&
                      ob(iob)%name, aqu_d(iaq)
            end if
          end if
        end if
        ...
        if (time%end_mo == 1) then
          ...
          if (pco%aqu%m == "y") then
            ...
            if (pco%csvout == "y") then
              write (2525,'(*(G0.3,:","))')  time%day, time%mo, time%day_mo, time%yrc, iaq, ob(iob)%gis_id,&
                      ob(iob)%name, aqu_m(iaq)
            endif
          end if
          aqu_m(iaq) = aquz
        end if
        ...
        if (time%end_yr == 1) then
          ...
          if (pco%aqu%y == "y") then
            ...
            if (pco%csvout == "y") then
              write (2526,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, iaq, ob(iob)%gis_id,&
                      ob(iob)%name, aqu_y(iaq)
            end if
          end if
      ...
      if (time%end_sim == 1 .and. pco%aqu%a == "y") then
        ...
        if (pco%csvout == "y") then 
          write (2527,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, iaq, ob(iob)%gis_id, ob(iob)%name,&
                  aqu_a(iaq)
        end if 
      end if

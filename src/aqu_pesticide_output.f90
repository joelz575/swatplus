      ...
      !! daily print
        if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%pest%d == "y") then
             write (3008,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), aqupst_d(j)%pest(ipest)   !! pesticide balance
             if (pco%csvout == "y") then
               write (3012,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                       ob(iob)%name, cs_db%pests(ipest), aqupst_d(j)%pest(ipest)
             end if
          end if
        end if
        ...
        if (time%end_mo == 1) then
          ...
          !! monthly print
           if (pco%pest%m == "y") then
             write (3009,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), aqupst_m(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (3013,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), aqupst_m(j)%pest(ipest)
               end if
           end if
        ...
        if (time%end_yr == 1) then
          ...
          !! yearly print
           if (time%end_yr == 1 .and. pco%pest%y == "y") then
             write (3010,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&
                     cs_db%pests(ipest), aqupst_y(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (3014,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id,&
                         ob(iob)%name, cs_db%pests(ipest), aqupst_y(j)%pest(ipest)
               end if
           end if
          !! reset pesticide at start of next time step
          stor_init = aqupst_d(j)%pest(ipest)%stor_final
         ...
         if (time%end_sim == 1 .and. pco%pest%a == "y") then
           ...
           write (3011,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name,&

...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	!!Particle size distribution of sediment yield
	  sanyld(j) = sedyld(j) * hru(j)%sol%det_san    !! Sand yield
	  silyld(j) = sedyld(j) * hru(j)%sol%det_sil    !! Silt yield
	  clayld(j) = sedyld(j) * hru(j)%sol%det_cla    !! Clay yield
	  sagyld(j) = sedyld(j) * hru(j)%sol%det_sag    !! Small Aggregate yield
	  lagyld(j) = sedyld(j) * hru(j)%sol%det_lag    !! Large Aggregate yield

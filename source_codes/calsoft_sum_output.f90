      ...
      real :: ha_hru           !          |
          ...
          do ireg = 1, db_mx%lsu_reg
            do ilu = 1, region(ireg)%nlum
              ...
              do ihru_s = 1, region(ireg)%num_tot
                ...
                if (lscal(ireg)%lum(ilu)%meas%name == hru(ihru)%lum_group_c .or. lscal(ireg)%lum(ilu)%meas%name ==&
                        "basin") then
            ...
            do ilu = 1, region(ireg)%nlum
              if (lscal(ireg)%lum(ilu)%ha > 1.e-6) then
                ...
                lscal(ireg)%lum(ilu)%precip_aa = lscal(ireg)%lum(ilu)%precip_aa + lscal(ireg)%lum(ilu)%precip / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%pet_aa = lscal(ireg)%lum(ilu)%pet_aa + lscal(ireg)%lum(ilu)%pet / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%aa%srr = lscal(ireg)%lum(ilu)%aa%srr + lscal(ireg)%lum(ilu)%sim%srr / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%aa%lfr = lscal(ireg)%lum(ilu)%aa%lfr + lscal(ireg)%lum(ilu)%sim%lfr / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%aa%pcr = lscal(ireg)%lum(ilu)%aa%pcr + lscal(ireg)%lum(ilu)%sim%pcr / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%aa%etr = lscal(ireg)%lum(ilu)%aa%etr + lscal(ireg)%lum(ilu)%sim%etr / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
                lscal(ireg)%lum(ilu)%aa%tfr = lscal(ireg)%lum(ilu)%aa%tfr + lscal(ireg)%lum(ilu)%sim%tfr / &
                        (10. * lscal(ireg)%lum(ilu)%ha)
          ...
          do ireg = 1, db_mx%lsu_reg
            do ilu = 1, region(ireg)%nlum
              ...
              do ihru_s = 1, region(ireg)%num_tot
                ...
                !if (lscal(ireg)%lum(ilu)%lum_no == hru(ihru)%lum_group_c) then
                  ...
                  lscalt(ireg)%lum(ilu)%sim%srr = lscalt(ireg)%lum(ilu)%sim%srr + &
                          (10. * ha_hru * hltwb_y(ihru)%surq_gen)
            ...
            do ilu = 1, region(ireg)%nlum
              if (lscalt(ireg)%lum(ilu)%ha > 1.e-6) then
                ...
                !! convert back to mm, t/ha, kg/ha
                lscalt(ireg)%lum(ilu)%precip_aa = lscalt(ireg)%lum(ilu)%precip_aa + lscalt(ireg)%lum(ilu)%precip / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
                lscalt(ireg)%lum(ilu)%aa%srr = lscalt(ireg)%lum(ilu)%aa%srr + lscalt(ireg)%lum(ilu)%sim%srr / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
                lscalt(ireg)%lum(ilu)%aa%lfr = lscalt(ireg)%lum(ilu)%aa%lfr + lscalt(ireg)%lum(ilu)%sim%lfr / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
                lscalt(ireg)%lum(ilu)%aa%pcr = lscalt(ireg)%lum(ilu)%aa%pcr + lscalt(ireg)%lum(ilu)%sim%pcr / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
                lscalt(ireg)%lum(ilu)%aa%etr = lscalt(ireg)%lum(ilu)%aa%etr + lscalt(ireg)%lum(ilu)%sim%etr / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
                lscalt(ireg)%lum(ilu)%aa%tfr = lscalt(ireg)%lum(ilu)%aa%tfr + lscalt(ireg)%lum(ilu)%sim%tfr / &
                        (10. * lscalt(ireg)%lum(ilu)%ha)
        ...
        if (cal_codes%chsed == "y" .and. cal_codes%sed == "n" .and. cal_codes%plt == "n" .and. cal_codes%hyd_hru == "n"&
                .and. cal_codes%hyd_hrul == "n") then

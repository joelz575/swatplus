      subroutine hru_constituent_output(ihru)
      ...
      !! daily print
        if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%wb_hru%d == "y") then
             write (2000,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpest_bal(j)%pest(ipest)   !! pesticide balance
             if (pco%csvout == "y") then
               write (4015,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpest_bal(j)%pest(ipest)
             end if
          end if
        end if
        !! check end of month
        if (time%end_mo == 1) then
          ...
          !! monthly print
           if (pco%wb_hru%m == "y") then
             write (2001,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_m(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2005,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_m(j)%pest(ipest)
               end if
           end if
        ...
        if (time%end_yr == 1) then
          ...
          !! yearly print
           if (time%end_yr == 1 .and. pco%wb_hru%y == "y") then
             write (2002,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_y(j)%pest(ipest)
               if (pco%csvout == "y") then
                 write (2006,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_y(j)%pest(ipest)
               end if
           end if
...
!!!!! average annual print
         if (time%end_sim == 1 .and. pco%wb_hru%a == "y") then
           ...
           write (2003,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_a(j)%pest(ipest)
           if (pco%csvout == "y") then
             write (2007,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpestb_a(j)%pest(ipest)
           end if
...
104   format (4i6,2i8,2x,a,27f18.3)
      ...
      end subroutine hru_constituent_output